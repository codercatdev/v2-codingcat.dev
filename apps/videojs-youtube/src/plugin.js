...
/**
 * YouTube Controller - Wrapper for YouTube API
 *
...
class YouTube extends Tech {
  ...
  constructor(options, ready) {
    ...
    if (typeof document !== "undefined") {
      this.loadScript("https://www.youtube.com/iframe_api", this.apiLoaded);
      this.injectCss();
    }
  }

  /**
   * TODO: Should this be moved outside of the class?
   */
  apiLoaded() {
    YT.ready(function () {
  ...
  setSrc(source) {
    ...
    this.source = source;
    this.url = this.parseUrl(source.src);
 ...
 * For now making an assumption of yes.
 *
 * @return {boolean}
 *         - True if YouTube media is supported.
 *         - False if YouTube media is not supported.
 */
YouTube.isSupported = function () {
  //TODO: Check if this is a browser?
  return true;
};
 ...
 */
YouTube.canPlaySource = function (srcObj, options) {
  return srcObj?.type && YouTube.canPlayType(srcObj?.type);
};
 ...
 * @return {string} 'probably', 'maybe', or '' (empty string)
 */
YouTube.canPlayType = function (type) {
